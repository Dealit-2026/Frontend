...
export default function RegisterScreen({ onBack, onComplete, mode, initialData }: { onBack: () => void; onComplete: () => void; themeColor: string; mode: 'regular' | 'auction'; initialData?: any }) {
  const [showDraftModal, setShowDraftModal] = useState(false);
  const [showLoadDraftModal, setShowLoadDraftModal] = useState(false);
  const [saleType, setSaleType] = useState<'regular' | 'auction'>(initialData?.type || mode);
  const [title, setTitle] = useState(initialData?.name || '');
  const [price, setPrice] = useState(initialData?.price ? initialData.price.replace(/[^0-9]/g, '') : '');
    ...
    <div className="flex flex-col h-full relative bg-white">
      <div className="h-16 flex items-center px-4 border-b border-gray-50">
        ...
        <h1 className="flex-1 text-center font-bold text-base">
          {isEditMode ? '상품 수정' : (saleType === 'auction' ? '경매 등록' : '상품 등록')}
        </h1>
        {!isEditMode && (
          <button onClick={() => setShowDraftModal(true)} className="text-sm font-medium text-gray-400 px-2">임시저장</button>
        )}
      </div>
        ...
        <div className="space-y-3">
          ...
          <div className="flex space-x-3 overflow-x-auto no-scrollbar">
            <button className="w-20 h-20 bg-gray-50 rounded-2xl flex flex-col items-center justify-center space-y-1 shrink-0">
              ...
              <span className="text-xs font-bold text-gray-300">{images.length}/10</span>
            </button>
            {images.map((img, idx) => (
              <div key={idx} className="w-20 h-20 rounded-2xl overflow-hidden shrink-0 relative">
        ...
        <div className="space-y-8">
          ...
          <div className="space-y-2">
            <div className="relative border-b border-gray-200">
              ...
              />
              {/* AI 추천 빤짝이 아이콘 부분 */}
              <span className="absolute right-0 top-1/2 -translate-y-1/2 text-[10px] flex items-center" style={{ color: themeColor }}>
                <Sparkles size={10} className="mr-1" fill="currentColor" /> AI 추천
              </span>
            </div>
          </div>
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              ...
              <button className="px-3 py-1.5 border border-gray-100 rounded-lg text-xs font-bold text-gray-600">자주 쓰는 문구</button>
            </div>
            <div className="relative">
              <textarea 
                ...
                maxLength={2500}
              ></textarea>
              <span className="absolute bottom-4 right-4 text-[10px] text-gray-300 font-bold">{description.length}/2500</span>
            </div>
          </div>
          ...
          <div className="space-y-5">
            <div className="space-y-1">
              <h3 className="font-bold text-base">{saleType === 'auction' ? '경매 설정' : '가격 설정'}</h3>
              <span className="text-xs text-gray-600 block pb-1">{saleType === 'auction' ? '시작가' : '판매가'}</span>
              <div className="relative border-b border-gray-200">
                <input 
                  type="text" 
                  placeholder="₩ 가격" 
            ...
            {saleType === 'auction' && (
                <div className="space-y-2">
                  <span className="text-xs text-gray-300 font-bold">경매 기간 / 날짜 설정</span>
                  <div className="w-full h-12 bg-gray-50 rounded-2xl flex items-center justify-between px-4 cursor-pointer">
                    <span className="text-base text-gray-400 font-medium">2026.06.11</span>
          ...
          <>
            ...
            >
              <div className="text-center space-y-2">
                ...
                <p className="text-sm text-gray-500">작성 중인 내용을 저장하시겠습니까?</p>
              </div>
          ...
          <>
            ...
            >
              <div className="text-center space-y-2">
                ...
                <p className="text-sm text-gray-500">이어서 작성하시겠습니까?</p>
              </div>
              <div className="flex space-x-3">
                <button 

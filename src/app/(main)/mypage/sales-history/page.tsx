...
export default function SalesHistoryScreen({ onBack, themeColor }: { onBack: () => void; themeColor: string; key?: string }) {
  const [selectedItem, setSelectedItem] = useState<any>(null);

  const sales = [
    { 
      id: 'SEL-20260324-01', 
      date: '2026.03.24 10:20', 
      item: '아이패드 프로 4세대', 
      price: 850000, 
      status: '판매완료',
      image: 'https://images.unsplash.com/photo-1544244015-0df4b3ffc6b0?auto=format&fit=crop&q=80&w=200',
      history: [
        { time: '2026.03.20 09:00', action: '상품 등록', detail: '경매 시작' },
        { time: '2026.03.22 21:00', action: '경매 종료', detail: '낙찰 성공' },
        { time: '2026.03.23 10:30', action: '구매자 결제', detail: '결제 완료' },
        { time: '2026.03.23 15:00', action: '상품 발송', detail: '우체국택배' },
        { time: '2026.03.24 10:20', action: '구매 확정', detail: '정산 예정' },
        { time: '2026.03.25 10:00', action: '정산 완료', detail: '계좌 입금' },
      ]
    },
    { 
      id: 'SEL-20260318-02', 
      ...
      history: [
        { time: '2026.03.15 14:00', action: '상품 등록', detail: '즉시 구매' },
        { time: '2026.03.16 11:20', action: '구매자 결제', detail: '결제 완료' },
        { time: '2026.03.16 17:00', action: '상품 발송', detail: 'CJ대한통운' },
        { time: '2026.03.18 16:40', action: '구매 확정', detail: '정산 예정' },
        { time: '2026.03.19 10:00', action: '정산 완료', detail: '계좌 입금' },
      ]
    },
  ];
        ...
        {sales.map((sale) => (
          <div key={sale.id} className="bg-white p-4 rounded-xl shadow-sm border border-gray-100">
            <div className="flex justify-between items-center mb-3 pb-3 border-b border-gray-50">
              <span className="text-xs text-gray-400 font-mono">{sale.date}</span>
              <span className="text-xs font-medium" style={{ color: themeColor }}>{sale.status}</span>
            </div>
            <div className="flex space-x-4">
              <img src={sale.image} alt={sale.item} className="w-16 h-16 object-cover rounded-lg bg-gray-100" />
              <div className="flex-1 flex flex-col justify-center">
                <span className="font-medium text-gray-800 line-clamp-1">{sale.item}</span>
                <span className="font-bold mt-1">{sale.price.toLocaleString()}원</span>
              </div>
            </div>
            <button 
              onClick={() => setSelectedItem(sale)}
